//E:\MyDelphiComponent\EhLib_v4.2.16_CN\EhLib 4.2.16 Install\Delphi2007/DBGridEH.pas
{*******************************************************}
{                                                       }
{                    EhLib v4.2                         }
{                                                       }
...
//    Nothing to do    
    else if (Abs(FDownMousePos.X - X) > 3) and (dgColumnResize in Options) then
    begin
      StopTracking;
...
var
  BetweenRowHeight,{xieyunc-->恢复被注释掉的变量BetweenRowHeight,}  t: Integer;
...
(*========================================================================*)
    ...
    begin
      ...
      if FFooterRowCount = 0 then  //xieyunc-->增加的一个条件，用户可以用FooterRowCount值来决定是否填满表格线
      ...
      begin
        ...
        ContraRowCount := FooterRowCount;
        //xieyunc-->恢复原代码中注释掉的部分
        (*========================================================================*)
        NewRowCount := NewRowCount + FooterRowCount + 1;
        SetRowCount(NewRowCount);
        BetweenRowHeight := ClientHeight - OccupiedHeight {- LineHeight};
        if BetweenRowHeight < 0 then BetweenRowHeight := 0;
        //RowHeights[TopDataOffset + VisibleDataRowCount] := BetweenRowHeight;//xieyunc-->这一行仍然不要，改成下面的代码
        (*========================================================================*)
       RowHeights[TopDataOffset + VisibleDataRowCount] := 0;//xieyunc-->修改后的代码行，替换上面仍然不要的那行
       //xieyunc-->增加下面一行代码，把多余的不足一行的高度加在最后一行
