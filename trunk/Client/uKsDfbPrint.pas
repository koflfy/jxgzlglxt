...
var
  KsDfbPrint: TKsDfbPrint;

implementation
uses uDM,uAutoPkInput;
{$R *.dfm}

procedure TKsDfbPrint.btn1Click(Sender: TObject);
begin
  dm.PrintReport('考场安排表.fr3',cds_KcRoom.XMLData,1);
end;

procedure TKsDfbPrint.btn2Click(Sender: TObject);
begin
  dm.PrintReport('考场指示标签.fr3',cds_KcRoom.XMLData,1);
end;

procedure TKsDfbPrint.btn3Click(Sender: TObject);
begin
  dm.PrintReport('考场座位标签.fr3',cds_Zkz.XMLData,1);
end;

procedure TKsDfbPrint.btn_AddClick(Sender: TObject);
begin
...
procedure TKsDfbPrint.btn_AutoCreateClick(Sender: TObject);
begin
{
...
procedure TKsDfbPrint.btn_ExitClick(Sender: TObject);
begin
  //DM.ExecSql(mmo1.Text);
  Close;
end;

procedure TKsDfbPrint.btn_PrintAllDfbClick(Sender: TObject);
var
  sqlstr:string;
begin
  if MessageBox(Handle,
    '真的要打印所有考场的考生登分表吗？　', '系统提示',
  ...
  try
    sqlstr := 'select * from view_考生签到表  ORDER BY 考场编号, 显示顺序, 准考证号';
    cds_Temp.XMLData := DM.OpenData(sqlstr);
    dm.PrintReport('考生登分表.fr3',cds_Temp.XMLData,1);
  finally
    Screen.Cursor := crDefault;
  end;
end;

procedure TKsDfbPrint.btn_PrintClick(Sender: TObject);
var
  sUrl,sqlstr:string;
begin
...
procedure TKsDfbPrint.btn_prnDfbClick(Sender: TObject);
var
  sqlstr:string;
begin
  sqlstr := 'select * from view_考生签到表 WHERE 考场编号='+quotedstr(cds_KcRoom.FieldByName('考场编号').AsString)+' ORDER BY 考场编号,显示顺序, 准考证号';
  cds_Temp.XMLData := DM.OpenData(sqlstr);
  dm.PrintReport('考生登分表.fr3',cds_Temp.XMLData,1);
end;

procedure TKsDfbPrint.btn_PrnZkz2Click(Sender: TObject);
begin
...
procedure TKsDfbPrint.btn_RefreshClick(Sender: TObject);
begin
  OpenTable_KcRoom;
end;

procedure TKsDfbPrint.cds_TempCalcFields(DataSet: TDataSet);
var
  Photo_fn:string;
begin
...
procedure TKsDfbPrint.chk_1Click(Sender: TObject);
begin
  OpenTable;
end;

procedure TKsDfbPrint.DBGridEh_ZkzDblClick(Sender: TObject);
begin
  aForm.Show;
end;

procedure TKsDfbPrint.ds_KcRoomDataChange(Sender: TObject; Field: TField);
begin
  OpenTable_KsKc;
  OpenTable;
end;

procedure TKsDfbPrint.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TKsDfbPrint.FormCreate(Sender: TObject);
begin
  aForm := TKsInfo.Create(Self);
  aForm.DataSource1.DataSet := Self.cds_Zkz;
end;

procedure TKsDfbPrint.FormDestroy(Sender: TObject);
begin
  aForm.Free;
end;

procedure TKsDfbPrint.FormShow(Sender: TObject);
begin
  OpenTable_KcRoom;
end;

procedure TKsDfbPrint.OpenTable;
var
  sqlstr:string;
begin
...
procedure TKsDfbPrint.OpenTable_KcRoom;
var
  sqlstr:string;
begin
...
procedure TKsDfbPrint.OpenTable_KsKc;
var
  sqlstr:string;
begin
...
procedure TKsDfbPrint.PageControl1Change(Sender: TObject);
begin
  OpenTable;
end;
