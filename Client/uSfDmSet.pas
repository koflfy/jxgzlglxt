...
procedure TSfDmSet.btn_AddClick(Sender: TObject);
begin
...
procedure TSfDmSet.btn_DelClick(Sender: TObject);
begin
...
procedure TSfDmSet.btn_ExitClick(Sender: TObject);
begin
  close;
end;

procedure TSfDmSet.btn_SaveClick(Sender: TObject);
begin
...
procedure TSfDmSet.ClientDataSet1NewRecord(DataSet: TDataSet);
begin
  DataSet.FieldByName('显示顺序').AsInteger := DataSet.RecordCount+1;
end;

procedure TSfDmSet.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TSfDmSet.FormCreate(Sender: TObject);
begin
  OpenTable;
end;

procedure TSfDmSet.OpenTable;
var
  i: Integer;
begin
  sqlStr := 'select * from  省份代码表 order by 显示顺序';
  ClientDataSet1.XMLData := dm.OpenData(sqlStr);
end;

procedure TSfDmSet.PageControl1Change(Sender: TObject);
begin
  OpenTable;
end;
